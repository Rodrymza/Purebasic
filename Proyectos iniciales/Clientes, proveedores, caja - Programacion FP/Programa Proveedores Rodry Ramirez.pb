...
Global NewList lista_clientes.datos() : Global NewList lista_provedores.datos()
AddElement(lista_clientes())
lista_clientes()\id="100001"
lista_clientes()\nombre="Consumidor Final"
lista_clientes()\domicilio="---"
lista_clientes()\correo="---"

Procedure.s generacionClave()
  contador=contador+1
  largo=6
  clave.s=Str(contador)
  For i=Len(clave) To largo-1
    clave=Str(Random(9)) + clave
  Next
  ProcedureReturn clave
EndProcedure

Procedure actualizarVentanaIngreso(List lista.datos())
  
    SetGadgetText(#codigo_text, lista()\id)
...
Procedure ventanaListaClientes(List lista.datos())
...
Procedure grabarDatos(List lista.datos(),texto.s)
  If texto=clientes : texto="cliente" : Else : texto="provedor" : EndIf 
  AddElement(lista())
  ForEach lista()
  ...
  If cont>0
    ...
    MessageRequester("Error","Ya existe un " + texto + " con el Cuil/DNI: (" + lista()\nombre + ")", #PB_MessageRequester_Error)
    SetGadgetText(#codigo_text,lista()\id)
    SetGadgetText(#nombre_text,lista()\nombre)
    SetGadgetText(#domicilio_text,lista()\domicilio)
    SetGadgetText(#correo_text,lista()\correo)
...
Procedure ventanaingreso(List lista.datos(),label$)
      ...
      Case #PB_Event_Gadget
        Select EventGadget()
          ...
          Case #boton_modificar
            ...
            lista()\domicilio=GetGadgetText(#domicilio_text)
            lista()\correo=GetGadgetText(#correo_text)
            MessageRequester("Exito","Datos modifiados correctamente",#PB_MessageRequester_Ok)
        EndSelect
      Case #PB_Event_CloseWindow
